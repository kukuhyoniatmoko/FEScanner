...
import com.foodenak.itpscanner.utils.executeTransaction
...
class EventRepository @Inject constructor(context: Context, val eventDao: EventEntityDao, val eventImageDao: EventImageEntityDao, val historyDao: HistoryEntityDao) {
  ...
  fun deleteAllEvent() {
    eventDao.database.executeTransaction {
      val entities = eventsQuery.forCurrentThread().list()
  ...
  fun saveEvents(events: List<Event>) {
    eventDao.database.executeTransaction { events.forEach { event -> saveEventInternal(event) } }
  }

  fun saveEvent(event: Event): Long {
    return eventDao.database.executeTransaction { saveEventInternal(event) }
  }
