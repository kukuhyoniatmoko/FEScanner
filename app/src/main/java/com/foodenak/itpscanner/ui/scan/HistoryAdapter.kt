...
class HistoryAdapter(val imageLoader: ImageLoader) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
  ...
  fun setFooter(type: Int, show: Boolean, resetOther: Boolean) {
    ...
    if (!hasFooter && !show) return
    when (type) {
      FOOTER_LOADING -> {
        if (show || resetOther) showResultNotFound = false
        showProgress = show
      }
      FOOTER_NOT_FOUND -> {
        if (show || resetOther) showProgress = false
  ...
  override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder? {
    ...
    when (viewType) {
      ...
      TYPE_PROGRESS ->
        return ProgressListItemHolder(
            inflater.inflate(R.layout.list_item_history_progress, parent, false))
      TYPE_NOT_FOUND ->
        return ResultNotFoundHolder(
            inflater.inflate(R.layout.list_item_result_not_found, parent, false))
      else ->
