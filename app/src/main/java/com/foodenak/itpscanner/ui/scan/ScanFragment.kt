...
class ScanFragment : Fragment(), FragmentCompat.OnRequestPermissionsResultCallback, IScanResultHandler, ScanView {
    ...
    private fun replaceWithCheck() {
        if (ContextCompat.checkSelfPermission(context, ScanActivity.PERMISSIONS[0]) == PackageManager.PERMISSION_GRANTED) {
            replace();
        } else {
            if (ActivityCompat.shouldShowRequestPermissionRationale(activity, ScanActivity.PERMISSIONS[0])) {
        ...
        when (requestCode) {
            ScanActivity.PERMISSIONS_REQUEST -> {
                if (grantResults.isNotEmpty() && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                    ...
                    Toast.makeText(context, R.string.camera_permission_denied_message, Toast.LENGTH_LONG).show()
                    if (isResumed) {
                        replaceDenied()
                    }
                }
    ...
    override fun showEditRedeemOptions(user: User) {
        ...
        val snack = Snackbar.make(scrollView, snackText.toString(), Snackbar.LENGTH_LONG)
                .setAction(R.string.edit, View.OnClickListener {
                    viewModel.editRedeemOptionsListener.call(user)
                })
        snack.setDuration(TimeUnit.SECONDS.toMillis(10).toInt())
        snack.show()
    }
